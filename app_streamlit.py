...
# --- パスワード認証機能 ---
def check_password():
    # SessionCookieがあれば認証済みとして扱う（F5更新時対策）
    if st.context.cookies.get("auth_session") == "true":
        return True
    ...
    if "password_correct" not in st.session_state:
        ...
        # パスワード正解時にCookieを発行してF5更新に備える (ブラウザを閉じると消えるセッションCookie)
        components.html(
            """
            <script>
                window.parent.document.cookie = "auth_session=true; path=/";
            </script>
            """,
            height=0,
            width=0
...
def render_shifted_page(pdf_bytes, page_index, mode, limit_to_cells, shift_amount, v_shift_amount):
    ...
    try: page.insert_font(fontname="msmincho", fontfile=r"C:\Windows\Fonts\msmincho.ttc")
    except:
        try: page.insert_font(fontname="msmincho", fontbuffer=fitz.Font("cjk").buffer)
        except: pass
        
    for item in extracted_items:
        rx0, ry0, rx1, ry1 = item["rect"]
        page.insert_text(fitz.Point(max(rx0 - shift_amount, 2), ry1 + v_shift_amount), item["text"], fontsize=item["size"], fontname="msmincho", render_mode=0)
